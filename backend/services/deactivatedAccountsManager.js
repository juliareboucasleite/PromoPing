...
export class DeactivatedAccountsManager {
  ...
  static async checkAndDeleteExpiredAccounts() {
    try {
      ...
      if (expiredAccounts.length === 0) {
        console.log(`[DEACTIVATED_ACCOUNTS] Nenhuma conta expirada encontrada`);
        return { deleted: 0, accounts: [] };
      }
      
      console.log(`[DEACTIVATED_ACCOUNTS] Encontradas ${expiredAccounts.length} contas para deletar`);
      ...
      for (const account of expiredAccounts) {
        try {
          ...
          try {
            ...
            deletedAccounts.push({ id: userId, email: account.Email });
            console.log(`[DEACTIVATED_ACCOUNTS] Conta ${userId} (${account.Email}) deletada automaticamente`);
          } catch (error) {
      ...
      console.log(`[DEACTIVATED_ACCOUNTS] ${deletedAccounts.length} contas deletadas com sucesso`);
      return { deleted: deletedAccounts.length, accounts: deletedAccounts };
    } catch (error) {
  ...
  static async startAutomaticCheck() {
    console.log('[DEACTIVATED_ACCOUNTS] Iniciando verificação automática de contas desativadas');
